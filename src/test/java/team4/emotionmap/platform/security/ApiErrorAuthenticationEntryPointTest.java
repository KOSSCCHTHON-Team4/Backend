...
class ApiErrorAuthenticationEntryPointTest {
    ...
    private static final ServiceConfigSource CONFIG = () -> new ServiceConfig(
            "test",
            1,
            new GeoPoint(1, 1),
            new ServiceLimits(
    ...
    private MockHttpServletResponse run(String authorization) throws Exception {
        ...
        new JwtAuthenticationFilter(
                provider,
                (userId, path) -> fail("Invalid JWT must not reach account access"),
                writer
        ).doFilter(
                request,
                response,
                (req, res) -> entryPoint.commence(
                        request,
                        response,
    ...
    void expiredTokenIsTokenExpired() throws Exception {
        String expiredToken = Jwts.builder()
                ...
                .issuedAt(Date.from(Instant.EPOCH))
                .expiration(Date.from(Instant.EPOCH.plusSeconds(60)))
                .signWith(Keys.hmacShaKeyFor(SECRET.getBytes(StandardCharsets.UTF_8)))
                .compact();

        MockHttpServletResponse response = run("Bearer " + expiredToken);
    ...
    void garbageTokenIsInvalidToken() throws Exception {
        MockHttpServletResponse response = run("Bearer not.a.jwt");
    ...
    void signedTokenWithoutSubjectIsInvalidToken() throws Exception {
        ...
        MockHttpServletResponse response = run("Bearer " + token);
    ...
    void validTokenAuthenticatesWithCurrentAccountRole() throws Exception {
        MockHttpServletRequest request = request(
                "GET",
                "/v1/config",
                "Bearer " + provider.createToken(USER_ID).token()
        );
        MockHttpServletResponse response = new MockHttpServletResponse();
    ...
    void onboardingDenialStopsRequestAndUsesStructuredError() throws Exception {
        ...
        new JwtAuthenticationFilter(provider, guard, writer)
                .doFilter(
                        request,
                        response,
    ...
    void deletedAccountReturnsInvalidTokenRatherThanGenericForbidden() throws Exception {
        ...
        new JwtAuthenticationFilter(provider, (userId, path) -> {
            throw new ResponseStatusException(HttpStatus.UNAUTHORIZED, "INVALID_TOKEN");
                ...
                request,
                response,
                (req, res) -> fail("Deleted account must be denied")
        );
    ...
    void loginIgnoresStaleBearerTokenAndAccountGuard() throws Exception {
        ...
        new JwtAuthenticationFilter(
                provider,
                (userId, path) -> fail("Public login must bypass account access"),
                writer
    ...
    void userContextAcceptsOnlyUuidPrincipals() {
        SecurityContextUserContext context = new SecurityContextUserContext();
        ...
        assertThat(context.currentUserId()).contains(USER_ID);
        SecurityContextHolder.getContext().setAuthentication(
                new org.springframework.security.authentication.TestingAuthenticationToken(
                        42L,
                        null,
