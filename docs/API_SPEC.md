...
| 로그인 5회 실패·1분 제한 | [구현] 영속 키·창·초기화 계약 확정 | N·L은 서비스 설정, W는 별도 필수 운영 설정. 5·60은 예시이며 운영값을 확정하지 않음 |
...
**현재 비밀번호 저장:** `email_password_credentials`에 계정별 이메일·정규화 조회키·BCrypt 해시를 저장한다. 원문·복호화 가능한 비밀번호·단순 SHA-256 비밀번호는 저장하지 않는다. `PASSWORD_BCRYPT_STRENGTH`는 라이브러리 범위 4–31에서 명시하며 운영 비용은 별도 실측·승인 대상이다. `local`·`ci`의 비용 4는 합성 테스트용이며 `prod`를 함께 활성화해도 운영 환경값을 대신하지 못한다. [W7]

Spring Security의 `PasswordEncoder`로 생성·검증하며 기존 해시의 내장 비용을 존중하고 로그인 중 자동 재해시는 하지 않는다. 비밀번호는 trim·대소문자 변환·Unicode 정규화 없이 사용한다. 비어 있지 않은 유효 UTF-8 최대 72바이트만 허용하며 초과·잘못된 surrogate는 `422 VALIDATION_ERROR`다. 문자열의 문자 수와 UTF-8 바이트 수를 혼동하지 않는다. 비밀번호·해시는 요청·응답·로그에 노출하지 않는다. [W8]

이메일은 최초 공급과 로그인 모두 `strip` 후 `Locale.ROOT` 소문자화한 뒤 동일한 이메일 검증을 적용한다. 제공자별 점·플러스 주소 제거는 하지 않는다. 최초 공급 CLI는 명시적 DB·스키마·비용 설정과 숨김 콘솔 또는 승인된 프로세스의 stdin을 요구한다. 신규 USER만 원자적으로 생성하며 ACTIVE/PENDING을 명시하고, 중복 이메일의 비밀번호·상태·제한을 덮어쓰지 않는다. 위치·취향을 자동 생성하지 않는다. 실행 절차는 `ARCHITECTURE.md` 7.1을 따른다.

이메일의 잘못된 surrogate는 대체 문자로 바꾸지 않고 로그인에서 `422 VALIDATION_ERROR`로 거절한다. 최초 공급도 같은 검증을 적용하며 잘못된 식별자로 계정·자격증명·제한 행을 생성하지 않는다.
...
**현재 영속 제한 계약:** 공개 서비스 설정의 실패 한도 N·잠금 초 L과 별도 양수 `LOGIN_FAILED_ATTEMPT_WINDOW_SECONDS` W를 사용한다. FE 예시 5·60은 운영값이 아니다. 정규화 이메일의 도메인 분리 SHA-256 키를 PostgreSQL 행으로 잠그므로 등록 여부·서버 재시작·인스턴스 수와 무관하게 같은 제한을 적용한다. 이 키는 가명화이며 사전 공격에 안전한 익명 식별자는 아니다. 최초 실패부터 고정 W 안에서 집계하고 N번째 실패부터 `429 TOO_MANY_ATTEMPTS`다. 잠금 중에는 올바른 비밀번호도 429이며 창·기한을 연장하지 않는다. 만료된 창·잠금은 다음 시도에서 초기화한다. 실제 행 잠금과 해시 검증 이후 DB 시각으로 결정하며 응답 `retryAfterSeconds`와 `Retry-After`는 남은 초의 올림값으로 일치한다.

올바른 비밀번호는 계정 상태 403을 반환하더라도 카운터 초기화를 먼저 커밋한다. 없는 이메일은 프로세스별 dummy 해시로 검증하되 성공으로 취급하지 않는다. 입력 오류·DB/검증기 장애는 실패 횟수에 포함하지 않는다. 필수 인증 설정 또는 W 누락·오류는 `503 CONFIGURATION_UNAVAILABLE`이며 카운터를 변경하지 않는다. IP 등 별도의 남용 방어가 구현됐다는 뜻은 아니다. [W3]
...
이메일·비밀번호로만 로그인하며 계정을 자동 생성하지 않는다. 정규화 이메일의 PostgreSQL 영속 제한을 통과한 뒤 비밀번호와 현재 계정 상태를 확인한다. N·L·W, N번째 429, 잠금 비연장, 성공·상태 거절 시 초기화 및 서버 장애 비집계는 3.2의 현재 구현 계약을 따른다. 실제 운영 수치는 별도 승인 대상이다.
...
**주요 오류 코드:** `INVALID_CREDENTIALS`, `VALIDATION_ERROR`, `TOO_MANY_ATTEMPTS`, `INVITATION_REQUIRED`, `ACCOUNT_SUSPENDED`, `ACCOUNT_CLOSED`, `INVALID_JSON`, `DUPLICATE_JSON_KEY`, `INVALID_REQUEST`, `CONFIGURATION_UNAVAILABLE`, `SERVICE_UNAVAILABLE`。
...
22개 참여자용 엔드포인트·공통 스키마·성공과 오류 예시를 같은 계약으로 정리했다. 실제 FE 저장소는 읽지 못했으므로 현재 Zod·훅과 실행 수준의 호환을 검증한 것은 아니다. 이후 구현한 영속 로그인 제한·내부 공급·이미지 제한은 별도 합성 런타임 검증을 거쳤지만 이 문서의 모든 API가 구현됐다는 뜻은 아니다.
...
초안 작성 당시 YAML 파싱, 22개 API·48개 스키마·207개 로컬 참조, 354개 정상 요청/응답 예시, 19개 부정 입력, 310개 명세 내 예시를 검사했다. 이메일·비밀번호 단일 요청·외부 토큰 요청 거절·응답 이메일 null 거절·고정 auth.mode와 TypeScript `--strict --noEmit` 검사도 통과했다. 오류 예시의 반복을 포함한 당시 수치이며 실제 354개 API 호출을 수행한 것은 아니다. 이 정적 검사 이력은 `validation/validation_report.json`을 따른다. 이후 기능별 실제 런타임 검증과 구분하며 실제 AI 호출의 증거로 사용하지 않는다.
