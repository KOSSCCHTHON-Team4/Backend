...
이번 기본 DDL에는 PostGIS·pgvector가 필요 없다. 서버의 단일 거리 함수로 미터 단위를 통일하는 안을 제안한다. 좌표를 단순히 빼거나 PostgreSQL point의 좌표 단위 거리를 미터라고 취급하지 않는다. 규모가 커져 PostGIS geography·GiST를 도입한다면 별도 결정·마이그레이션으로 진행한다. 거리식·정밀도·고정 반경 수치는 현재 문서에서 임의 확정하지 않는다.
